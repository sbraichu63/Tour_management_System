...
def api_packages(request):
    ...
    for p in posts:
        packages.append({
            ...
            'size': p.Size,
            'hoster': p.Hoster,
            'accommodation': p.Accomodation,
            'transportation': p.Transportation,
            'image_url': request.build_absolute_uri(p.pack_img.url) if p.pack_img else None,
            'detail_url': reverse('pack_detailView', args=[p.pk]),
            'checkout_url': reverse('checkout', args=[p.pk]),
        })
