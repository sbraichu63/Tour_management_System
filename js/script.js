...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ==================== DYNAMIC PACKAGES FROM BACKEND API ====================
    function renderPackages(packages) {
        const container = document.querySelector('.packages .box-container');
        if (!container) return;
        ...
        packages.forEach(pkg => {
            ...
            image.innerHTML = `<img src="${pkg.image_url || '/static/images/default-tour.jpg'}" alt="${pkg.title}">`;

            const content = document.createElement('div');
            content.className = 'content';
            content.innerHTML = `
                <h3>${pkg.title}</h3>
                <p>${pkg.body ? pkg.body.replace(/<[^>]*>/g, '').slice(0, 160) + '...' : 'No description'}</p>
                <div class="price">NPR ${pkg.price}</div>
                <div class="meta">Days: ${pkg.days} | Group: ${pkg.size} | ${pkg.provinces}</div>
                <a href="${pkg.detail_url}" class="btn">Explore now</a>
            `;
        ...
        fetch('/api/packages/')
            ...
            .then(data => {
                if (data && Array.isArray(data.packages)) {
                    ...
                    console.error('Unexpected packages format', data);
                }
            })
            .catch(error => {
                console.error('Error loading packages from API', error);
                showNotification('Unable to load packages right now, showing default packages', 'error');
            });
    }
